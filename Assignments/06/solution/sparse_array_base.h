...
public:
  ...
  constexpr index_type pos() const { return _pos; }
...
public:
  ...
  constexpr difference_type operator+(const derived_t &rhs) const {
  ...
  bool operator==(const self_t &rhs) const noexcept { return _pos == rhs._pos; }
  bool operator!=(const self_t &rhs) const noexcept { return !(*this == rhs); }
  constexpr reference operator*() const { return derived().dereference(); }
  proxy_reference operator*() { return derived().dereference(); }
...
public:
  // base
  proxy_reference dereference() { return proxy_reference(*_array, _index); }
...
public:
  ...
  size_t size() const { return _sizeA; }
  size_t max_size() const { return size(); }

  // Iterators
  iterator begin() { return iterator(this, 0); }
  const iterator end() { return iterator(this, size()); }
  ...
  const_reference operator[](index_t index) const { return sparseValue(index); }

  proxy_reference operator[](index_t offset) {
    return proxy_reference(*this, offset);
  }
  const_reference at(index_t index) const { return sparseValue(index); }
