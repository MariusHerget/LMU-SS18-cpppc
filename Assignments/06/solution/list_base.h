...
public:
  ...
  template <class Derived> class ForwardIteratorBase {
  private:
  ...
  public:
    ...
    const ValueT &operator*() const { return derived().dereference(); }
    ValueT &operator*() { return derived().dereference(); }
    bool operator!=(const self_t rhs) const { return !(*this == rhs); }
    bool operator==(const self_t &rhs) const { return (this == &rhs); }
  };

  // template <class T>
  class ListIterator : public ForwardIteratorBase<ListIterator> {
  ...
  public:
    ...
    value_type &dereference() { return _node->value; }
    void increment(int offset) {
      for (int i = 0; i < offset; i++)
        _node = _node->next;
    }
    bool operator==(const self_t &rhs) const {
      return (base_t::operator==(rhs) || (_node == rhs._node));
    }
...
public:
  ...
  ~list() = default;
  // MOVE
  list(self_t &&other) : _head(other._head), _size(other._size) {
