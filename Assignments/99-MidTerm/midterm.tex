...
\item
  ...
  members)?
  \problemAnswer{
        Potential memory leak when the if condition throws an error.\\ The concept needs to fullfill the Rule of Three/Five.
...
\item
  ...
  \problemAnswer{
    Resource Allocation is Initialization (RAII) is a common technique which depends on this rule. In this technique the ressources are allocated during the initialization (via constructor) and hold until the object is destroyed (via destructor.)
  }
\end{itemize}
...
\note{All mentioning of rules (e.g. \texttt{Rule 1.}) are based on \url{https://en.cppreference.com/w/cpp/container#Thread_safety}}
